...
app.config['DATABASE_URI'] = 'postgres://oywegrrpomcpcn:ea81d69c7715ff882408d8d6fda1203402fc8c5b4e302d10defbddc13692538b@ec2-34-234-240-121.compute-1.amazonaws.com:5432/d8umna7v86uqav'
...
def eat_healthy():
    ...
    try:
        response = requests.get("https://api.edamam.com/search?q=" + search_word + "&app_id=a8f807ca&app_key=9e763f1edd4c3c936eb2506f1dbdddf5&from=0&to=40&calories=591-722&cuisineType=Indian")
...
def show_saved_recipes():
    ...
    if saved_recipes is not None:
        for row in saved_recipes:
            ...
            try:
                response = requests.get("https://api.edamam.com/search?r=" + r_id + "&app_id=a8f807ca&app_key=9e763f1edd4c3c936eb2506f1dbdddf5&from=0&to=40&calories=591-722&cuisineType=Indian")
            except HTTPError as http_err:
                print(f"HTTP error occurred: {http_err}")
            except Exception as err:
                print(f"An error ocurred: {err}")
...
def get_toned():
    ...
    data = open("./static/poses.json", "rb")
    pose = json.load(data)
    return render_template("get_toned.html", pose=pose)
